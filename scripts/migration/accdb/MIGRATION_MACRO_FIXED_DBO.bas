...
' Результат: Все совпадающие таблицы мигрируются в FormulaD на SQL Server
' ВАЖНО: Эта версия работает с таблицами названными dbo_tblRole, dbo_tblUser и т.д.
' ============================================================================

Option Compare Database
Option Explicit

Sub MigrateDataToSQLServer_DBO()
    ...
    reportText = reportText & "МИГРАЦИЯ ДАННЫХ В SQL SERVER (dbo_ версия)" & vbCrLf
    ...
    recordsInserted = MigrateTable(db, "dbo_tblSpecifications", "tblSpecifications", _
        "INSERT INTO dbo_tblSpecifications (id, specificationName, valid, ModelArticle, id_assembly) " & _
