...
' ============================================================================
' Запустить в Access базе (MES_server.accdb)
' ============================================================================
...
Sub MigrateDataToSQLServer()
    ...
    recordsInserted = MigrateTable(db, "tblConnections", _
        "INSERT INTO tblConnections (id, [Критические соединения], [Кол-во соединений], Станция, Код_колмплектации, [Станция проверки]) " & _
        ...
        "WHERE NOT EXISTS (SELECT 1 FROM tblConnections AS dst WHERE dst.id = src.Код)")
    totalRecords = totalRecords + recordsInserted
    reportText = reportText & "tblConnections: " & recordsInserted & " записей" & vbCrLf
    ...
    recordsInserted = MigrateTable(db, "tblNCP", _
        ...
        "SELECT ID, StationID, ComponentID, VIN, Quantity, DiscrepancyTypeID, Comment, Status, CreatedBy, CreatedAt, ClosedBy, ClosedAt FROM tblNCP AS src " & _
    ...
    recordsInserted = MigrateTable(db, "tblDiscrepancy", _
        ...
        "SELECT ID, AssemblyEventID, ComponentID, DiscrepancyType, Comment, WasReplaced, DetectionTime, ResolutionTime, DiscrepancyStatus, NCP_Number, PhotoPath FROM tblDiscrepancy AS src " & _
    ...
    recordsInserted = MigrateTable(db, "tblImportLog", _
        ...
        "SELECT ID, ImportDate, FileName, ImportedBy, RowsImported, RowsFailed, ErrorDescription, ImportBatchID FROM tblImportLog AS src " & _
    ...
    reportText = reportText & vbCrLf
    reportText = reportText & "=" & String(80, "=") & vbCrLf
    reportText = reportText & "ИТОГО: " & totalRecords & " записей" & vbCrLf
    reportText = reportText & "Время: " & Format(Now() - startTime, "hh:mm:ss") & vbCrLf
    ...
    Exit Sub
ErrorHandler:
    MsgBox "Ошибка: " & Err.Description & vbCrLf & vbCrLf & "Номер строки: " & Err.Number, vbCritical
End Sub
...
Private Function MigrateTable(db As Object, tableName As String, sql As String) As Long
    ...
    Exit Function
ErrHandler:
    MsgBox tableName & ": " & Err.Description, vbCritical
    MigrateTable = 0
End Function
